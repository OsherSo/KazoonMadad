...
router.post("/calc", async function (req, res) {
  const data = req.body;
  const result = await calcExpectedGoals(
    data.league,
    data.homeTeam,
    data.awayTeam
  );
...
router.get(`/admin/up/${Base64.encode(key)}/:league`, async function (
  req,
  res
) {
  const league = req.params.league;
...
router.get(`/admin/del/${Base64.encode(key)}/:league`, async function (
  req,
  res
) {
  const league = req.params.league;
